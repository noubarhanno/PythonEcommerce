...
from PythonEcommerceProject.utils import unique_order_id_generator
...
class Order(models.Model):
    ...
    order_id            = models.CharField(max_length=120, blank=True) #AB13FED
    # Billing_Profile = ?
    ...
    # Meta class is to order the orders list from new one to old and without it the ordering will be from old(in top) to new (in bottom)
    class Meta:
...
class ProductPurchaseQuerySet(models.query.QuerySet):
    def active(self):
        return self.filter(refunded=Flase)
...
class ProductPurchase(models.Model):
    ...
    billing_profile = models.ForeignKey(BillingProfile, null=True, blank=True) #billingprofile.productpurchase_set.all()
